...
Дата и место проведения, статус и кнопка экспорта в календарь `.ics`.

![Карточка UFC 332 с экспортом в календарь](docs/screenshots/event.jpg)
...
Сервер получает `/ufc/events/upcoming` и `/ufc/events/recent` при первом открытии и затем по запросу пользователя, только если кеш старше 12 часов. Обычно это не более 4 запросов в сутки (124 за 31 день). Навигация по месяцам, поиск и экспорт не вызывают Cito API. Фонового опроса нет.
